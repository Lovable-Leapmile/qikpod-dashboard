...
const PodDetail: React.FC<PodDetailProps> = ({
  ...
  return <div className="space-y-6">
      ...
      <Card className="bg-white shadow-sm rounded-xl border-gray-200">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-start space-x-4">
              <div className="w-32 h-40 bg-yellow-400 rounded-lg flex items-center justify-center flex-shrink-0">
                <img alt="Qikpod Logo" src="/lovable-uploads/0fd687e7-e1ab-48e0-9dc6-fca54d66bec7.png" className="w-10 h-auto" />
              </div>
        ...
        <CardContent className="pb-8">
          ...
          <div className="mb-6">
            ...
            {/* Hide/Show Toggle */}
            <Button variant="ghost" onClick={() => setShowHiddenSection(!showHiddenSection)} className="flex items-center space-x-2 text-gray-600 mb-4">
      ...
      <Card className="bg-white shadow-sm rounded-xl border-gray-200">
        <CardHeader className="pb-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-3">
              <Button variant="outline" className="rounded-lg" onClick={() => setShowUpdateVersionPopup(true)}>
                <ArrowUp className="w-4 h-4 mr-2" />
                Update Pod Version
              </Button>
              <Button variant="outline" className="rounded-lg" onClick={() => setShowFEUpdatePopup(true)}>
        ...
        <CardContent>
          {logsLoading ? <div className="text-center py-12">
              <p className="text-gray-500">Loading logs...</p>
            </div> : logs.length === 0 ? <NoDataIllustration title="No logs found" description="There are no logs available for this pod." /> : <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {logs.map(log => <TableRow key={log.log_id}>
      ...
      {/* Popups */}
      <EditModePopup open={showEditModePopup} onOpenChange={setShowEditModePopup} podId={podId} initialValue={podDetail.pod_mode} onSuccess={handlePopupSuccess} />

      <UpdatePodVersionPopup open={showUpdateVersionPopup} onOpenChange={setShowUpdateVersionPopup} podId={podId} onSuccess={handlePopupSuccess} />

      <FEUpdatePopup open={showFEUpdatePopup} onOpenChange={setShowFEUpdatePopup} podId={podId} initialValues={{
