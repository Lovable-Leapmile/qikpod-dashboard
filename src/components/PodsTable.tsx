    ...
    <Card className="bg-white shadow-sm rounded-lg">
      <CardHeader className="pb-2 pt-3 px-3 rounded-t-lg bg-gray-50">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
          <CardTitle className="text-lg font-semibold text-gray-900 flex items-center">
            <Package className="w-4 h-5 mr-1 text-[#1f2937]" />
            Pods
          </CardTitle>
      ...
      <CardContent className="p-3">
          {isMobile ? <div className="space-y-2">
            ...
            </div> : <div className="ag-theme-alpine w-full rounded-xl overflow-hidden border border-gray-200" style={{
          height: 320
        }}>
              <AgGridReact rowData={pods} columnDefs={columnDefs} defaultColDef={defaultColDef} loading={loading} onGridReady={onGridReady} animateRows={true} suppressCellFocus={true} suppressRowClickSelection={true} rowHeight={55} headerHeight={60} pagination={true} paginationPageSize={25} paginationPageSizeSelector={[10, 25, 50, 100]} suppressPaginationPanel={false} suppressColumnVirtualisation={true} rowClass="cursor-default" suppressMenuHide={true} />
            </div>}
        </CardContent>
    </Card>
  );
