...
const NotificationsPage: React.FC = () => {
  ...
  const fetchSMSData = useCallback(async () => {
    ...
    try {
      const response = await fetch("https://productionv36.qikpod.com/notifications/notifications/sms/?order_by_field=updated_at&order_by_type=DESC", {
        headers: {
  ...
  const fetchEmailData = useCallback(async () => {
    if (!accessToken) return;
    try {
      const response = await fetch("https://productionv36.qikpod.com/notifications/notifications/email/?order_by_field=updated_at&order_by_type=DESC", {
        headers: {
          Authorization: `Bearer ${accessToken}`,
          Accept: "application/json"
        }
