...
const NotificationsPage: React.FC = () => {
  ...
  useEffect(() => {
    if (autoRefresh) {
      intervalRef.current = setInterval(refreshData, 2 * 60 * 1000);
    } else {
  ...
  return (
    <Layout title="Notification Centre" breadcrumb="Notifications">
      <div className="space-y-6">
        {/* Compact Header Controls - Matching LocationsTable style */}
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:w-auto">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {/* Auto Refresh Checkbox - Fixed styling */}
                      ...
                      <Card key={sms.id} className="bg-white shadow-sm rounded-xl border-gray-200">
                        <CardContent className="p-4">
                          <div className="flex justify-between items-start mb-3">
                            ...
                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                              sms.sms_delivery_status === 'delivered' ? 'bg-green-100 text-green-800' :
                              sms.sms_delivery_status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
                              'bg-red-100 text-red-800'
                            }`}>
                      ...
                      <Card key={email.id} className="bg-white shadow-sm rounded-xl border-gray-200">
                        <CardContent className="p-4">
                          <div className="flex justify-between items-start mb-3">
                            ...
                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                              email.email_delivery_status === 'delivered' ? 'bg-green-100 text-green-800' :
                              email.email_delivery_status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
                              'bg-red-100 text-red-800'
                            }`}>
